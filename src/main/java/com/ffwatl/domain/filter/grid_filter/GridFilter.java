package com.ffwatl.domain.filter.grid_filter;
...
public abstract class GridFilter {
    ...
    protected List<GridFilterRule> fillRule(String field, String longString){
        ...
        for(String s: values){
            list.add(new GridFilterRule().setData(s).setField(field));
            if(field.equals("locale_en")) list.add(new GridFilterRule().setData(s).setField("locale_ru"));
        }
        return list;
    }
}