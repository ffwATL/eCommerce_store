 ...
 * @author ffw_ATL.
 */
@Service("activity_state_manager")
public class ActivityStateManagerImpl implements ActivityStateManager {
    ...
    protected Map<String, Stack<StateContainer>> stateMap = new ConcurrentHashMap<>();
    ...
    @Override
    public void registerState(Activity<? extends ProcessContext> activity, ProcessContext processContext,
    ...
    protected RollbackStateLocal getRollbackStateLocal() {
        ...
        if (rollbackStateLocal == null) {
            throw new IllegalThreadStateException("Unable to perform ActivityStateManager operation, " +
                    "as the RollbackStateLocal instance is not set on the current thread! ActivityStateManager " +
                    "methods may not be called outside the scope of workflow execution.");
        }
